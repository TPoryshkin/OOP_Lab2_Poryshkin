...
class Program
{
    ...
    static void AddPlant()
    {
        ...
        try
        {
            Plant newPlant = CreatePlantManually();
            plants.Add(newPlant);
    ...
    static void DemonstrateBehavior()
    {
        ...
        if (int.TryParse(Console.ReadLine(), out int index) && index > 0 && index <= plants.Count)
        {
            ...
            Console.Write("Введіть ріст для методу Grow (у метрах): ");
            if (double.TryParse(Console.ReadLine(), out double growth))
            {
                try
                {
                    plant.Grow(growth);
            ...
            {
                Console.WriteLine("Некоректне значення росту.");
            }

            Console.Write("Бажаєте полити рослину? (так/ні): ");
